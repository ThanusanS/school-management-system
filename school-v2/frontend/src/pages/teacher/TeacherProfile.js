...
export default function TeacherProfile() {
  ...
  const handlePwChange = async e => {
    ...
    try {
      ...
      toast.success('Password changed!'); setPwModal(false);
  ...
  const fields = [
    ['Teacher ID', profile.teacherId], ['Email', profile.email], ['Phone', profile.phone || '-'],
    ['Subject', profile.subject], ['Qualification', profile.qualification || '-'],
    ['Experience', `${profile.experience} years`], ['Gender', profile.gender || '-'],
    ...
    ['Join Date', new Date(profile.joinDate).toLocaleDateString()],
    ['Classes', profile.assignedClasses?.join(', ') || '-'],
  ];
  ...
  return (
    <div>
      ...
      <div className="profile-hero" style={{ marginBottom: 24, background: 'linear-gradient(135deg, #10b981, #059669)' }}>
        <div className="avatar avatar-xl" style={{ background: 'rgba(255,255,255,0.2)' }}>{profile.firstName[0]}{profile.lastName[0]}</div>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 800 }}>{profile.firstName} {profile.lastName}</div>
          <div style={{ opacity: 0.85, marginTop: 4 }}>{profile.subject} Teacher</div>
          <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
            <span style={{ background: 'rgba(255,255,255,0.2)', padding: '4px 12px', borderRadius: 100, fontSize: 12 }}>👩‍🏫 {profile.teacherId}</span>
            <span style={{ background: 'rgba(255,255,255,0.2)', padding: '4px 12px', borderRadius: 100, fontSize: 12 }}>{profile.status}</span>
          </div>
        </div>
      </div>

      <div className="card" style={{ padding: '20px 24px' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, marginBottom: 16, fontSize: 16 }}>👤 Professional Information</div>
